...
class Ping(SonarSensor):
    ...
    def __init__(self,*args, **kwargs):
        ...
        self.msg.field_of_view = 0.5
        self.msg.min_range = 0.02
        self.msg.max_range = 4.5
        self.trig_pin = kwargs['trig_pin']
        self.echo_pin = kwargs['echo_pin']
        rospy.loginfo("Ping sensor init, trig:%d echo:%d" %(self.trig_pin,self.echo_pin))

    def read_value(self):
        
        # The Arduino Ping code returns the distance in centimeters
        cm = self.controller.ping(self.trig_pin,self.echo_pin)
