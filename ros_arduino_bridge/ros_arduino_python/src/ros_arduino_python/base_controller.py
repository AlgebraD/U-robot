...
class BaseController:
    def __init__(self, arduino, base_frame, name="base_controllers"):
        ...
        self.stopped = False
        self.debug_pid = rospy.get_param("~debug_pid", False)      
        ...
        rospy.loginfo("Started base controller for a base of " + str(self.wheel_track) + "m wide with encoder_resolution_A:" + str(self.encoder_resolution_A) + " encoder_resolution_B:"+ str(self.encoder_resolution_B)+ " encoder_resolution_C:"+ str(self.encoder_resolution_C))
    ...
    def setup_pid(self, pid_params):
        ...
        self.encoder_resolution_A = pid_params['encoder_resolution_A']
        self.encoder_resolution_B = pid_params['encoder_resolution_B']
        self.encoder_resolution_C = pid_params['encoder_resolution_C']
        self.gear_reduction_A = pid_params['gear_reduction_A']
        self.gear_reduction_B = pid_params['gear_reduction_B']
        self.gear_reduction_C = pid_params['gear_reduction_C']
    ...
    def poll(self):
        if self.debug_pid:
            try:
                A_pid_in, B_pid_in, C_pid_in = self.arduino.get_pid_in()
                self.AEncoderPub.publish(A_pid_in)
                self.BEncoderPub.publish(B_pid_in)
                self.CEncoderPub.publish(C_pid_in)
            except Exception,e:
                rospy.logerr(e.message)
                return

            try:
                A_pid_out, B_pid_out, C_pid_out = self.arduino.get_pid_out()
                self.APidOutPub.publish(A_pid_out)
                self.BPidOutPub.publish(B_pid_out)
                self.CPidOutPub.publish(C_pid_out)
            except:
                rospy.logerr("get_pid_out exception count")
                return    
        now = rospy.Time.now()
        if now > self.t_next:
            # Read the encoders
            ...
            # odom.pose.covariance[14] = 1.7976931348623158e+308
            # odom.pose.covariance[21] = 1.7976931348623158e+308
            # odom.pose.covariance[28] = 1.7976931348623158e+308
    ...
    def cmdVelCallback(self, req):
        ...
        tmpx = math.sqrt(3)/2.0
        tmpy = 0.5
        self.v_A = (tmpx*x + tmpy*y + self.wheel_track*th)
        self.v_B = (-tmpx*x + tmpy*y + self.wheel_track*th)
        self.v_C = (-y + self.wheel_track*th)
