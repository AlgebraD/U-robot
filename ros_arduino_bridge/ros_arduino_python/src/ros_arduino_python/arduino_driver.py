...
class Arduino:
    ...
    def drive(self, wheel_a, wheel_b, wheel_c):
        ''' Speeds are given in encoder ticks per PID interval
        '''
        return self.execute_ack('m %d %d %d' %(wheel_a, wheel_b ,wheel_c))
    ...
    def ping(self, trig_pin,echo_pin):
        ''' The srf05/Ping command queries an SRF05/Ping sonar sensor
            connected to the General Purpose I/O line pinId for a distance,
            and returns the range in cm.  Sonar distance resolution is integer based.
        '''
        return self.execute('p %d %d' %(trig_pin,echo_pin))
    
    def beep(self, beep_type=0):
        ''' The buzzer pin is defined in arduino. Stop beep if beep_type is -1,else  
            beep with the specific beep type.
        '''
        return self.execute_ack('B %d' %(beep_type))
    
    def temp_humi_read(self):
        ''' Read temperature and humidity
        '''
        return self.execute_array('T')
