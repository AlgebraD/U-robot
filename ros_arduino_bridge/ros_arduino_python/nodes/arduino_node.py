...
class ArduinoROS():
    def __init__(self):
        ...
        # A service to control buzzer
        rospy.Service('~beep',Beep,self.BeepHandler)

        # A service to read temperature and humidity
        rospy.Service('~temp_humi_read',TempHumiRead,self.TempHumiReadHandler)
        ...
        for name, params in sensor_params.iteritems():
            ...
            if params['type'] == "Ping":
                sensor = Ping(self.controller, name, params['pin'], params['rate'], 
                    params['frame_id'],trig_pin=params['trig_pin'],echo_pin=params['echo_pin'])
...
#                    self.sensors[len(self.sensors)]['output_pin'] = params['output_pin']
    ...
    def BeepHandler(self, req):
        value = self.controller.beep(req.beep_type)
        return BeepResponse()

    def TempHumiReadHandler(self, req):
        value = self.controller.temp_humi_read()       
        return TempHumiReadResponse(value[0],value[1])
